...
The target role is desktop support. For a long time the site argued for a SOC
job instead — security was folder `order: 1`, the PROJECTS channel wore a
shield, the hover bubble led with SIEM, three of the four Now beats were
detection engineering, and Contact offered four job titles across three
seniorities. Every one of those labs is good work; none of them is the pitch.

Infrastructure leads and security supports. The two strongest artefacts —
the helpdesk lab's five worked tickets with their KB articles, and the AD lab's
delegation of password resets to a tier-1 group and nothing else — are the case,
and they were buried under the security folder.

Folder order lives in `seed-copy.py`, not `merge_projects.py`. That file uses
`doc.setdefault("folders", ...)`, which seeds a fresh document and never touches
an existing one, so an order change there is silently ignored on every real run.

## Counting honestly

Thirteen labs: five security, eight infrastructure. The other six repos are side
builds. The site said "twenty" in three places and the card said twenty, while
the noscript block listed eighteen — an interviewer who opens a crypto
backtester expecting a documented IT lab notices. The OG card is generated by
`scripts/make-og-card.mjs` and **counts** the labs rather than being told the
number, which is why it was the last thing still claiming twenty.

## Evidence

`shots` and `metric` are optional per project and render nothing when absent, so
the panels look untouched until there is something real to show.
`notes/SHOTLIST.md` names the six captures worth taking and what to redact —
the build gate reads text and cannot read an image, so it can enforce nothing
about what is visible in a screenshot. `publish.sh` check 11 does enforce that
every referenced file exists and carries a real alt sentence.

The PDF is generated from `resume.html` and from nothing else, and check 12
pins the SHA of the source. That is stronger than grepping the PDF's text: it
proves the bytes came from a file the PII check has already read.
...
Opening a channel is a **FLIP from the tile's rect to the panel's**, not a fade.
`router.peekTrigger()` supplies the element that was pressed without consuming
it — the close path still needs the same element for focus restoration — so the
dock buttons and the rows inside an overlay animate from their own positions
without any extra wiring. Uniform scale from the width; a separate Y scale
visibly distorts a panel of text. With motion off, no animation object is
created at all and the close is synchronous, because a deferred exit that never
runs would strand a panel on screen.

The **entrance plays when the menu becomes visible**, not when it is built. It
used to fire while the opaque start screen was still up, which meant the best
animation on the site was invisible on a first visit — i.e. on every visit
arriving from a job application. That start screen now also lets itself out
after 1.8s; a key or a click still skips it instantly.
